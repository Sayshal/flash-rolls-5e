...
/**
 * Utility class for handling status effects in the Roll Requests Menu
 */
export class RollMenuStatusUtil {
  ...
  static async applyStatusToActor(statusEffect, actor) {
    try {
      ...
      if (existingEffect) {
        LogUtil.log(`RollMenuStatusUtil: Actor ${actor.name} already has status effect ${statusEffect.id}`);
        return false;
      }
      ...
      if (actor.toggleStatusEffect) {
        ...
        if (!hasEffect) {
          ...
          LogUtil.log(`RollMenuStatusUtil: Applied ${statusEffect.id} to ${actor.name}`);
          return true;
        }
        return false;
      }
      ...
      LogUtil.error(`RollMenuStatusUtil: Failed to apply status effect to ${actor.name}`, [error]);
      return false;
    }
  }
    ...
    try {
      ...
      if (actor.toggleStatusEffect) {
        ...
        if (hasEffect) {
          ...
          LogUtil.log(`RollMenuStatusUtil: Removed ${statusEffect.id} from ${actor.name}`);
          return true;
        }
        return false;
      }
      ...
      if (!existingEffect) {
        LogUtil.log(`RollMenuStatusUtil: Actor ${actor.name} does not have status effect ${statusEffect.id}`);
        return false;
      }
      ...
      LogUtil.error(`RollMenuStatusUtil: Failed to remove status effect from ${actor.name}`, [error]);
      return false;
    }
  }
  ...
  static getStatusEffectsForTemplate() {
    try {
      if (!CONFIG.statusEffects || !Array.isArray(CONFIG.statusEffects)) {
        LogUtil.warn('RollMenuStatusUtil: CONFIG.statusEffects not available or not an array');
        return [];
      }
      ...
      LogUtil.error('RollMenuStatusUtil: Error getting status effects for template', [error]);
      return [];
    }
  }
}